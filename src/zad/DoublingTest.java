...
import zad.Insertion;
import zad.IntInsertion;
import zad.Insertion.Comp;
...
public class DoublingTest {
    ...
    public static double timeTrial(int n) {
        ...
        DoublingTest com = new DoublingTest();
        for (int i = 0; i < n; i++) {
            a[i] = StdRandom.uniform(-MAXIMUM_INTEGER, MAXIMUM_INTEGER);
            b[i] = String.valueOf(a[i]);
        }
        Stopwatch timer = new Stopwatch();
        Insertion.sort(b,com.new Comp());
    ...
    public static double timeTrial_2(int n) {
    	...
    	Stopwatch timer = new Stopwatch();
		int[] sorted = IntInsertion.sortInt(a);
    	return timer.elapsedTime();
    }
    
    public class Comp implements Comparator<Object>{
    	public int compare(Object v, Object w) {
    		int vi = Integer.parseInt(v.toString());
    		int wi = Integer.parseInt(w.toString());		
    		return vi - wi;
    	}
    }
    ...
    public static void main(String[] args) { 
        ...
        for (int n = 0; n<=10; n++) {
            double time = timeTrial((int)Math.pow(2, n));
            double timen = timeTrial_2((int)Math.pow(2, n));
            StdOut.printf("%7d %f %f %f\n ----\n", (int)Math.pow(2, n), time, timen, timen/time);
        } 
    } 
}
